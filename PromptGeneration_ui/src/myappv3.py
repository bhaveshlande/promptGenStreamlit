...
def generate_better_prompt(input_prompt, temp, context, answer_prompt):
    try:
        ...
        for token in token_list:
            if str(token) != "\n":
                mat = np.append(arr, [str(token.text), str(token.tag_), str(token.dep_), str(token.head.text)],
                                axis=0)
        ...
        # prompt = f"I need a {named_entities_str} AI prompt based on the {input_prompt}, its {context}, and the user's {answer_prompt}. The prompt should be specific, clear, open-ended, and relevant. It should start with 'I want you to act as' and expand accordingly."
        model = TextGenerationModel.from_pretrained("text-bison@001")
        response = model.predict(prompt)
        return response

    except Exception as e:
        st.error(f"Error generating a better prompt: {str(e)}")
...
def ask_specific_question(input_prompt):
    try:
        ...
        st.error(f"Error asking a specific question: {str(e)}")
...
def evaluate_prompt(prompt):
    try:
        ...
        st.error(f"Error evaluating the prompt: {str(e)}")
...
def Launch():
    try:
        ...
        with st.form('addPrompt'):
            ...
            selected_option = st.select_slider("Select an Option for better output:",
            ...
            if st.form_submit_button('Next'):
                display_question_Ai(input_prompt, temp)

    except Exception as e:
        st.error(f"An error occurred: {str(e)}")


def display_question_Ai(input_prompt, temp):
    if not input_prompt:
        st.warning("Please enter a prompt before clicking 'Next'.")
    else:
        ...
        #         st.warning("Please enter an Answer for the question before Generating a prompt'.")
        if button_clicked2 is not None:
